...
function propose(
	...
	logdens(x) = logdensity(model, x)
	## collect the walkers's positions in a matrix
	walkers = hcat([walker.params for walker in walkers_t]...)
	...
	for set in sets
		...
		new_walkers[active] = [Transition(positions_shrink[:,i], logdensities_shrink[i], true, μ, t + 1) for i in 1:n_wdiv2]
	end
